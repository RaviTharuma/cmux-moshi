# Disclaimer and limitation of liability

**READ THIS BEFORE INSTALLING, USING, FORKING, CONTRIBUTING TO, OR RELYING ON
ANYTHING IN THIS REPOSITORY.**

This software is provided **free of charge**, as a **personal / hobby /
volunteer** open-source project. Publishing free code does **not** create a
product sale, paid support contract, employment, partnership, agency,
fiduciary duty, or customer relationship with the authors or maintainers.
**You are not buying anything. We owe you nothing beyond the MIT license grant.**

By cloning, downloading, building, installing, running, distributing,
star­ring, forking, filing issues, or contributing to **cmux-moshi**, you
acknowledge and agree to this document, the [MIT License](LICENSE),
[TERMS.md](TERMS.md), [NOTICE.md](NOTICE.md), and (if you contribute)
[CONTRIBUTING.md](CONTRIBUTING.md). If you do not agree, **do not use,
download, or rely on this software — delete it.**

## Unofficial project

**cmux-moshi** is an independent, community-maintained host integration. It is
**not** an official product of, and is **not** affiliated with, endorsed by, or
sponsored by:

- [cmux](https://cmux.com) / [manaflow-ai/cmux](https://github.com/manaflow-ai/cmux)
- [Moshi](https://getmoshi.app) or its publishers
- Apple, GitHub, crates.io, Cursor, Railway, cloud providers, or any other
  third party whose names appear in docs or tooling

“cmux”, “Moshi”, and other product names are trademarks of their respective
owners. They are used only to describe compatibility.

Authors and maintainers act in a **personal capacity** only (unless a separate
written contract you signed with them says otherwise — this repo is not that
contract).

## No warranty (absolute)

THE SOFTWARE, DOCUMENTATION, COMMENTS, ISSUES, PULL REQUESTS, RELEASE ASSETS,
CI ARTIFACTS, DEPENDENCIES, SCRIPTS, AND ANY ADVICE IN OR ABOUT THIS
REPOSITORY ARE PROVIDED **“AS IS” AND “AS AVAILABLE”**, WITHOUT WARRANTY OF
ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO WARRANTIES OF
MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, TITLE, NON-INFRINGEMENT,
SECURITY, ACCURACY, COMPLETENESS, OR QUIET ENJOYMENT.

**There is no warranty that the software is free of defects, malware,
backdoors, supply-chain compromise, accidental harmful logic, or contributor
error.** Maintainers and copyright holders do **not** promise to review,
audit, sanitize, test on your machines, or continuously monitor every commit,
pull request, dependency, action, or release binary. They may leave public
issues/PRs disabled, ignore submissions, or merge by mistake — **none of that
creates liability** for supply-chain harm, credit burn, or malicious code.

You may **not** rely on README text, issues, discussions, or chat answers as
professional, legal, security, or operational advice.

## Limitation of liability (maximum extent)

TO THE MAXIMUM EXTENT PERMITTED BY APPLICABLE LAW, THE AUTHORS, COPYRIGHT
HOLDERS, MAINTAINERS, AND THEIR HEIRS, SUCCESSORS, AND ASSIGNS SHALL **NOT BE
LIABLE** FOR ANY CLAIM, DEMAND, DAMAGES, COST, OR OTHER LIABILITY — WHETHER IN
CONTRACT, TORT (INCLUDING NEGLIGENCE), STRICT LIABILITY, STATUTE, OR
OTHERWISE — ARISING FROM, OUT OF, OR IN CONNECTION WITH THE SOFTWARE, THIS
REPOSITORY, OR THE USE OR OTHER DEALINGS IN THE SOFTWARE, **INCLUDING WITHOUT
LIMITATION**:

1. **Supply-chain and dependency risk** — compromised crates, GitHub Actions,
   runners, mirrors, tags, release binaries, checksums, package registries,
   transitive dependencies, or build tooling.
2. **Malicious or defective contributions** — code, docs, or CI changes
   submitted by third parties that a maintainer merges, accepts, or overlooks,
   whether or not review was attempted.
3. **Credits, quotas, and cloud spend** — overuse or unexpected consumption of
   API credits, CI minutes, cloud compute, bandwidth, Cursor/agent usage,
   LLM tokens, hosting, or any third-party billable resource triggered by
   installing, building, testing, running, or automating this project.
4. **Host damage** — data loss, session kill/rename, shell rc edits,
   LaunchAgent installs, credential exposure, downtime, or system compromise.
5. **Indirect / consequential / special / incidental / punitive loss** — lost
   profits, lost data, business interruption, reputational harm, or similar,
   even if advised of the possibility.
6. **Third-party or downstream claims** — claims by your employer, customers,
   users of your fork, or anyone who obtained the software through you.

IF ANY LIABILITY CANNOT BE FULLY EXCLUDED IN YOUR JURISDICTION, IT IS LIMITED
TO THE GREATER OF (A) **ZERO (US$0.00)** OR (B) THE MINIMUM AMOUNT REQUIRED BY
LAW. YOU ASSUME **ALL** RISK. **FREE CODE ≠ FREE LIABILITY FOR THE AUTHOR.**

## Covenant not to sue (to the max extent allowed)

To the maximum extent permitted by law, you agree **not to bring** any claim,
action, or proceeding against the authors or maintainers arising from your
download, use, inability to use, redistribution, or reliance on this free
software, except where such a waiver is void or prohibited. This is a
condition of being allowed to use the software.

## Your responsibilities

You alone are responsible for:

- Verifying source, commits, signatures, and checksums before use
- Pinning and auditing dependencies and CI for your threat model
- Backups, access control, and safe use of `sync` / `cleanup` / installers
- Any cloud, CI, or AI-agent spend incurred in your accounts
- Deciding whether to trust a release, fork, or contributor patch
- Compliance with laws in your jurisdiction

Prefer `--dry-run` where available. Treat every binary and PR as untrusted
until **you** have validated it.

## Forks and mirrors

If you fork, mirror, or redistribute this project, **you** become responsible
for what you publish. That does not create liability for the original authors.
Users of forks must look to the fork operator, not to this repository’s
authors, for that fork’s behavior.

## Security reports

Suspected vulnerabilities: [SECURITY.md](SECURITY.md) (private). Filing a
report does **not** create any duty, SLA, deadline, or liability for
maintainers.

## Scope reminder

Phones connecting over Mosh/SSH never see the cmux left sidebar. This repo’s
product is the **host CLI** (and optional packaging). See [AGENTS.md](AGENTS.md).

## Conflict

If anything in READMEs, issues, or chats conflicts with [LICENSE](LICENSE),
this file, [NOTICE.md](NOTICE.md), or [TERMS.md](TERMS.md), the license and
these liability terms control.
